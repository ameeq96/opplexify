...
import { Edit3, Eye, LogOut, Plus, RefreshCw, Save, Trash2, Upload } from "lucide-react";
import { API_URL, assetUrl } from "../../lib/api";

type Field = {
  name: string;
  label: string;
  type?: "text" | "textarea" | "json" | "number" | "checkbox" | "select" | "date" | "password" | "media";
  options?: string[];
  relation?: string;
  full?: boolean;
};
...
const resources: Resource[] = [
  {
    ...
    fields: [
      { name: "key", label: "Key" },
      { name: "value", label: "Value JSON", type: "json", full: true }
    ]
  },
  {
    key: "menus",
  ...
  },
  {
    ...
    fields: [
      ...
      { name: "gallery", label: "Feature List JSON", type: "json", full: true },
  ...
  },
  {
    ...
    fields: [
      ...
      { name: "gallery", label: "Gallery JSON", type: "json", full: true },
      { name: "videoUrl", label: "Video URL", type: "media" },
      { name: "contentBlocks", label: "Content Blocks JSON", type: "json", full: true },
      { name: "featured", label: "Featured", type: "checkbox" },
      { name: "sortOrder", label: "Sort Order", type: "number" },
      { name: "status", label: "Status", type: "select", options: statusOptions }
    ]
  ...
  },
  {
    ...
    fields: [
      ...
      { name: "socialLinks", label: "Social Links JSON", type: "json", full: true },
      { name: "skills", label: "Skills JSON", type: "json", full: true },
      { name: "sortOrder", label: "Sort Order", type: "number" },
      { name: "status", label: "Status", type: "select", options: statusOptions }
    ]
  },
...
export function AdminConsole() {
  ...
  const resource = useMemo(() => resources.find((item) => item.key === active) ?? resources[0], [active]);
  ...
  function startCreate() {
    ...
    resource.fields.forEach((field) => {
      ...
      if (field.name === "status") defaults[field.name] = "PUBLISHED";
  ...
  function startEdit(item: any) {
    ...
    resource.fields.forEach((field) => {
      ...
      if (field.type === "json" && resource.key === "page-sections" && field.name === "content") next[field.name] = value ?? {};
          ...
          <div className="admin-panel">
            <div className="admin-toolbar">
              <div>
                ...
                <p>Manage records through the NestJS admin API.</p>
              </div>
              <div className="inline-actions">
              ...
              <form className="form" onSubmit={submit}>
                <div className="admin-form-grid">
                  {resource.fields.map((field) => {
                    ...
                    return resource.key === "page-sections" && field.name === "content" ? (
...
function SectionContentEditor({
  ...
  return (
    <div className="admin-field full section-content-editor">
      ...
      {!["hero", "text-media", "rich-text", "pricing", "work-showcase", "service-showcase", "team-showcase", "logo-strip", "capability-list", "stats", "contact-info"].includes(type) ? (
        <>
